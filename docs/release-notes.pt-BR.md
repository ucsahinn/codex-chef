...
Esta versao amplia a entrega publica do repo com uma knowledge base bilingue,
um novo validador de locales para KB, exemplos de install-plan redigidos e uma
imagem social preview atualizada.

## v0.5.47 - 2026-07-09

Esta versao corrige um falso `fail` de `codex:status:all` encontrado depois da
verificacao de v0.5.46. Probes Codex/MCP mais lentos no Windows recebem mais
tempo para a verificacao de runtime, mas drift real em managed files, skills,
Git guards ou MCP continua falhando.

## v0.5.46 - 2026-07-09

Esta versao reduz approval prompts desnecessarios para verificacao segura e
release-readiness do Codex Chef. Validadores granulares, checks de skills e
runtime, package dry-runs, GitHub run watching e diagnosticos read-only do
Codex ficam allowlisted; repair apply, cleanup, publish, deploy, release,
credentials, mudancas de dependencias e package execution ad-hoc continuam
prompt-gated.

## v0.5.45 - 2026-07-09

Esta versao reforca os 21 agentes especialistas com um bloco
`World-class specialist upgrade` exigido por validadores. Cada agente nomeia o
failure mode do seu dominio, aplica evidence grading, executa um senior
challenge, preserva sua role boundary e encaminha trabalho adjacente ao
especialista correto. A validacao de package surface tambem bloqueia arquivos
untracked, ignored, local-state, `tmp` aninhado, build, coverage e artefatos de
dependencias dentro do npm pack.

## v0.5.44 - 2026-07-09

Esta versao adiciona `codebase-memory` como MCP habilitado por padrao, mas
limitado por allowlist de ferramentas para analise de codebase com grafo local.
As ferramentas read/query ficam allowlisted, indexing e ferramentas de
project-state mais arriscadas ficam prompt-gated ou desativadas, e os
artefatos gerados `.codebase-memory/` ficam fora do source control. O status
board agora tambem mostra controles de context-budget e o lembrete do perfil
`token-safe` para trabalhos longos no repo inteiro.

## v0.5.43 - 2026-06-25

Este patch leva a experiencia de operador da CLI para o menu inteiro e seus
submenus. Skills, MCP, backups, diagnostics, process audit, auth e telas de
preview agora usam a mesma estrutura com resumo primeiro, linguagem natural e
limites seguros de escrita do menu principal.
...
Este patch classifica skills globais nao curadas do usuario como nota no
relatorio de repair, nao como motivo de attention. Um runtime limpo permanece
`Overall: ok`, enquanto skills curadas ausentes, duplicadas e drift real seguem
visiveis.
