...
| App connector guvenligi config ve validation icinde acik olmali. | `templates/codex/config.windows.toml`, `templates/codex/config.unix.toml` ve `scripts/security-audit.mjs`; `apps._default.enabled = false`, `apps._default.destructive_enabled = false` ve `apps._default.open_world_enabled = false` alanlarini zorunlu tutar. | Tamam |
