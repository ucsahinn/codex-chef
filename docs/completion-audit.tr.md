...
| Uzman ajan metadata'si config template'leriyle ayni hizada kalmali. | `catalog/agents.json` incelenmis yirmi uzman ajani kaydeder; `scripts/validate-agent-config.mjs` hem Windows hem Unix `[agents.<name>]` bloklarini ve her `templates/codex/agents/*.toml` role dosyasini kontrol eder. | Tamam |
