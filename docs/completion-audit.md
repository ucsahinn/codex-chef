...
| Specialist agent metadata should stay aligned with config templates. | `catalog/agents.json` records the seven reviewed specialist agents, and `scripts/validate-agent-config.mjs` checks both Windows and Unix `[agents.<name>]` blocks plus each `templates/codex/agents/*.toml` role file. | Complete |
