...
| One-shot install should turn Codex into a strong specialist setup without surprising global Git changes. | `scripts/install.ps1 -All -Force` and `scripts/install.sh --all --force` install Codex templates, verified public/first-party skill packages, specialist agents, profiles, rules, and the local plugin; global Git guards stay behind the explicit `-InstallGitGuards` / `--install-git-guards` flag. | Complete |
