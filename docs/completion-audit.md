...
| One-shot install should turn Codex into a strong specialist setup without surprising global Git changes. | `scripts/install.ps1 -All` and `scripts/install.sh --all` install Codex templates, verified public/first-party skill packages, specialist agents, profiles, rules, and the local plugin; existing `config.toml` is merged without overwriting existing tables, and global Git guards stay behind the explicit `-InstallGitGuards` / `--install-git-guards` flag. | Complete |
...
| Specialist agent metadata should stay aligned with config templates. | `catalog/agents.json` records the reviewed specialist agents, and `scripts/validate-agent-config.mjs` checks both Windows and Unix `[agents.<name>]` blocks plus each `templates/codex/agents/*.toml` role file. | Complete |
...
| Requirement | Evidence | Status |
| --- | --- | --- |
| Token/context budgeting should be available without adding a risky external default. | `plugins/codex-chef-workflows/skills/context-budget-planner/SKILL.md` ships as a local plugin skill for token/context budgets, source prioritization, compaction handoff, and verification gates. | Complete |
| Extra design, Vercel, prompt, context, memory, and token-related capabilities should not disappear. | `catalog/skills.json`, `README.md`, `README.tr.md`, `docs/skills-and-agents.md`, and `docs/skills-and-agents.tr.md` keep `impeccable`, design-taste, Vercel, prompt, context, memory, and token-related skills visible as manual opt-in references. | Complete |
| First-party ecosystem skills should be available in the one-shot skill setup. | `catalog/skills.json`, `catalog/skills-lock.json`, `README.md`, `README.tr.md`, `docs/skills-and-agents.md`, and `docs/skills-and-agents.tr.md` include `context-engineering-project-starter`, `codex-enterprise-prompt-architect`, and `codex-skill-forge` in the reviewed `-All` / `-InstallSkills` set. | Complete |
| Default skill installation should remain bounded and verifiable. | `catalog/skills-lock.json` contains sixteen installable global skills; external overlapping design, Vercel, memory, token, prompt, and context references stay manual and are excluded from `-All` install operations. | Complete |
| Bundled local skills should be complete and hard to drift. | Every local plugin skill now has `SKILL.md`, `references/*.md`, and `agents/openai.yaml`; `scripts/validate-plugin-skills.mjs` and `npm run validate:plugin-skills` enforce the plugin manifest and catalog alignment. | Complete |
| Offline diagram claims should match the actual renderer. | `offline-diagram-triplet/references/diagram-contract.md` documents the supported Mermaid flowchart subset, Excalidraw scene contract, unsupported features, safety rules, and validation command. | Complete |
| GStack/ECC research should improve the starter without importing risky surfaces. | `docs/workflow-surface-map.md`, `docs/ecc-compatibility.md`, `docs/research-notes.md`, and the Codex templates record the adopted role/manifest/doctor/diagram patterns and the excluded cookie, tunnel, deploy, raw-CDP, hook-injection, and broad cross-harness sync behaviors. | Complete |
| App connector safety should be explicit in config and validation. | `templates/codex/config.windows.toml`, `templates/codex/config.unix.toml`, and `scripts/security-audit.mjs` require `apps._default.destructive_enabled = false` and `apps._default.open_world_enabled = false`. | Complete |
| Mojibake regressions should be a machine gate, not a manual screenshot concern. | `scripts/validate-content-safety.mjs` rejects likely UTF-8/Windows-1252 mojibake sequences in tracked text files while preserving normal Unicode and README emoji. | Complete |
| External high-star starter ideas should be synthesized, not copied. | All twenty-one `templates/codex/agents/*.toml` role files now include role-specific authority metadata, source-refresh, research-synthesis, adversarial-validation, source-currency, corpus-expansion, and expert-calibration rules. `catalog/agent-research-corpus.json` records each role's research domains, source types, authority metadata, refresh triggers, and handoffs; validators enforce one copy of each runtime contract and guardrail block, at least 100 source-backed instruction items, at least 20 distinct source markers per role, and authority keys that match role TOML source markers. `docs/workflow-surface-map.md` records how ECC/GStack patterns map to Codex-native surfaces or explicit exclusions. | Complete |
