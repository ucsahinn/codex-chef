...
| First-party ecosystem skills should be available in the one-shot skill setup. | `catalog/skills.json`, `catalog/skills-lock.json`, `README.md`, `README.tr.md`, `docs/skills-and-agents.md`, and `docs/skills-and-agents.tr.md` include `ai-project-starter`, `prompt-architect`, and `ai-skill-create` in the reviewed `-All` / `-InstallSkills` set. | Complete |
...
| App connector safety should be explicit in config and validation. | `templates/codex/config.windows.toml`, `templates/codex/config.unix.toml`, and `scripts/security-audit.mjs` require `apps._default.enabled = false`, `apps._default.destructive_enabled = false`, and `apps._default.open_world_enabled = false`. | Complete |
